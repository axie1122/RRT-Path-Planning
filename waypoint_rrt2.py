...
class RRT():
    def __init__(self, start, goal, bounds, thresh: int, factor):
        start_x = start[0]
        start_y = start[1]
    ...
    def add_node(self, x, y):
        ...
        node.cost = prev.cost + np.linalg.norm(np.array((x_n, y_n)) - prev_arr)

        if  self.bounds(node):
            cps, dists = self.get_n_closest_points(x_n, y_n)
            minCost = node.cost
            ind = -1
            for i in range(len(cps)):
                cost = cps[i].cost + np.linalg.norm(np.array((x_n, y_n)) - np.array(cps[i].as_tuple()))
                if cost <= minCost:
                    minCost = cost
                    ind = i
            if (ind == -1):
            ...
            node2 = Node(x_n, y_n, cps[ind])
            node2.cost = minCost
            if self.bounds(node2):
                node = node2
                prev = cps[ind]
                
            self.nodes.append(node)
            self.prev_coords[self.index - 1] = np.array(prev.as_tuple())
            self.rewire(cps, node)
    ...
    def rewire(self, cps, node):
        for i in cps:
            if i == self.nodes[0] or i == node.prev:
                continue
            cost = node.cost + np.linalg.norm(np.array(node.as_tuple()) - np.array(i.as_tuple()))
            if cost < i.cost:
                prev_node, prev_cost = i.prev, i.cost
                i.prev, i.cost = node, cost
                if not self.bounds(i):
                    i.prev, i.cost = prev_node, prev_cost
    ...
    def get_n_closest_points(self, x, y):
        delta_vec = self.coords[ : self.index] - np.tile(np.array((x, y)).reshape(1, 2), (self.index, 1))
        dist_vec = np.sum(delta_vec ** 2, 1)

        cp_indices = np.argwhere(dist_vec <= (((self.thresh*self.factor)**2) + 1)).flatten()
        # print(cp_indices)

        cps = np.array(self.nodes)[cp_indices]
        dists = np.array(dist_vec)[cp_indices]
        return cps, dists
    ...
    def __call__(self):
        ...
        self.total_cost = self.goal.prev.cost + np.linalg.norm(np.array(self.goal.as_tuple()) - np.array(self.goal.prev.as_tuple()))
...
class Boundaries():
    ...
    # def check_ out_bound_point(self, x, y):
